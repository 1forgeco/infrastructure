...
exports.handleGetMetrics = void 0;
...
const handleGetMetrics = async (req, res) => {
    ...
    try {
        ...
        const activeTenants = await prisma_1.prisma.tenantProfile.count({
            where: {
                org_id: orgId,
                is_active: true,
                status: "active",
        ...
        const unpaidDues = await prisma_1.prisma.due.findMany({
            where: {
                org_id: orgId,
                status: {
        ...
        const history = await prisma_1.prisma.platformMetric.findMany({
            where: whereHistory,
            orderBy: {
                metric_date: "asc",
            },
...
exports.handleGetMetrics = handleGetMetrics;
