...
exports.handleGetWardDetails = void 0;
const prisma_1 = require("../../../lib/prisma");
const handleGetWardDetails = async (req, res) => {
    const orgId = req.headers["x-org-id"];
    const parentUserId = req.user?.userId;
    try {
        // Find all parent profiles for this parent in the organization
        const parentProfiles = await prisma_1.prisma.parentProfile.findMany({
            where: {
        ...
        for (const profile of parentProfiles) {
            ...
            const activeDues = await prisma_1.prisma.due.findMany({
                where: {
            ...
            const activeGatePasses = await prisma_1.prisma.gatePass.findMany({
                where: {
                    tenant_id: wardUser.id,
                    org_id: orgId,
                    status: {
            ...
            if (profile.can_see_roommate_contact && tenantProfile.room_id) {
                const roommateProfiles = await prisma_1.prisma.tenantProfile.findMany({
                    where: {
...
exports.handleGetWardDetails = handleGetWardDetails;
